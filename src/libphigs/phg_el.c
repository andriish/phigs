    ...
    )
{
   ...
   if (PSL_STRUCT_STATE(PHG_PSL) != PSTRUCT_ST_STOP) {
      ...
      ARGS_ELMT_SIZE(&args) = 8 * sizeof(Pint);
      num_faces = vertex_data->num_lists;
      num_data_per_facet = facet_data->num_data_per_facet;
      if (facet_data->facet_colrv != NULL) {
         ARGS_ELMT_SIZE(&args) += num_faces * sizeof(Pcolrv);
      }
      if (facet_data->facet_norm != NULL) {
         ARGS_ELMT_SIZE(&args) += num_faces * sizeof(Pvec3);
      }
      if (facet_data->facet_data != NULL) {
         ARGS_ELMT_SIZE(&args) += num_faces * num_data_per_facet *
            sizeof(Pfloat);
      }

      ARGS_ELMT_SIZE(&args) += 1;
      num_edge_lists = edge_flags->num_lists;
      for (i = 0; i < num_edge_lists; i++) {
         num_edges = edge_flags->edge_flags[i].num_edges;
         ARGS_ELMT_SIZE(&args) += 1;
         ARGS_ELMT_SIZE(&args) += num_edges * sizeof(Pedge_flag);
      }

      switch(colr_type) {
         default:
            for (i = 0; i < num_faces; i++) {
               ARGS_ELMT_SIZE(&args) += 2;
               num_vertices = vertex_data->vertices[i].num_vertices;
               num_data_per_vertex =
                  vertex_data->vertices[i].num_data_per_vertex;
               if (vertex_data->vertices[i].vertex_colrvs.colr_reps.rgb !=
                   NULL) {
                  ARGS_ELMT_SIZE(&args) += num_vertices * sizeof(Pcolr_rep);
               }
               if (vertex_data->vertices[i].vertex_norms != NULL) {
                  ARGS_ELMT_SIZE(&args) += num_vertices * sizeof(Pvec3);
               }
               if (vertex_data->vertices[i].vertex_data != NULL) {
                  ARGS_ELMT_SIZE(&args) += num_vertices * num_data_per_vertex *
                     sizeof(Pfloat);
               }
            }
            break;
