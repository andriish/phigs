...
void glcContext(GLint context){
	if(!glcIsContext(context) || context == 0) {
		last_error = GLC_PARAMETER_ERROR;
	} else {
		currentGlcContext = context;
...
void glcDeleteContext(GLint context) {
	...
	for(j = 0; glcContextList[j] != 0; j++)
		if(glcContextList[j] == context) break;

	/* Delete all fonts in this context */
	for(i = glcContexts[context].num_fonts-1; i > 0; i--)
		glcDeleteFont(i);
